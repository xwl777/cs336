...
def count_word(text:str):
    ...
    word_bytes_cnt = defaultdict(int)
    ...
    for word, cnt in word_cnt.items():
        ...
        if len(word_bytes) >= 2: # 这个cnt计数器时用于后续方便计算pair出现的次数，长度为1的word_bytes不产生任何pair，故可以直接忽略
            word_bytes_cnt[word_bytes] += cnt
    return word_bytes_cnt
...
def count_pairs(word_cnt:dict):
    ...
    pairs2word = defaultdict(set) # 维护pair到word的反向索引，记录了pair存在于哪些word中
    for word,cnt in word_cnt.items():
        for pair in zip(word[:-1],word[1:]): #将两个可迭代对象打包成元组列表
            pairs_cnt[pair] += cnt
            pairs2word[pair].add(word)

    return pairs_cnt, pairs2word
...
# 合并pair后需要更新pair计数
def update_cnt(word_cnt:dict, pair_cnt:dict, pairs2word:dict[tuple[bytes, bytes], set[tuple[bytes, ...]]], merge_pair:tuple[bytes]):

    affected_words = pairs2word[merge_pair].copy()

    for word_bytes in affected_words:
        cnt = word_cnt[word_bytes]
        old_pairs = list(zip(word_bytes[:-1], word_bytes[1:]))

        # 将key更新为合并后的word_bytes，删除原key
        new_word_bytes = merge(word_bytes, merge_pair)
        del word_cnt[word_bytes]
        word_cnt[new_word_bytes] += cnt

        # 对每个old_pair，重新计算该词对pair_cnt的影响，将原计数全部删去后重新计数
        for pair in old_pairs:
            pair_cnt[pair] -= cnt
            if pair_cnt[pair] == 0:
                del pair_cnt[pair]
            pairs2word[pair].discard(word_bytes) # 在反向索引中删除原word

        for pair in zip(new_word_bytes[:-1], new_word_bytes[1:]):
            pair_cnt[pair] += cnt
            pairs2word[pair].add(new_word_bytes) # 在反向索引中加入新word
            if not pairs2word[pair]:
                del pairs2word[pair]
...
def train_bpe(input_path:str, vocab_size:int, special_tokens:list[str]):
    ...
    print("word 计数完成")
    pairs_cnt, pairs2word = count_pairs(word_cnt)
    print("配对计数完成")
    ...
    while(len(vocab) < vocab_size):
        ...
        word_cnt, pairs_cnt,pairs2word = update_cnt(word_cnt, pairs_cnt, pairs2word, merge_pair)
